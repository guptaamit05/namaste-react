...
import { Card, isOpenCloseRestaurant } from "./Card";
...
const Body = () => {
  ...
  const IsOpenOrCloseRest = isOpenCloseRestaurant(Card);
  useEffect(() => {
    ...
    <>
      <div className="text-lg m-5">
        <input
          ...
          className="focus:outline-none text-xl w-full p-2 border-2"
          onChange={(e) => {
            ...
            filteredArray.map((rest) => {
              return (
                <Link
                  ...
                  state={{ restData: rest.info }}
                  key={rest.info.id}
                >
                  {rest.info.isOpen ? (
                    <Card key={rest.info.id} oneRest={rest.info} />
                  ) : (
                    <IsOpenOrCloseRest key={rest.info.id} oneRest={rest.info} />
                  )}
                </Link>
              );
            })
